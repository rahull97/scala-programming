...
/*
    ...
    Does MyList support for comprehensions? = Yes
    For a collection to support for comprehensions,
    it requires specific function signatures for
    map, flatMap, and filter functions as described
    below:
    ...
    Thus they are implemented in the same way
    in MyList, therefore will support for
    comprehension
*/

object Lec_29_MyList extends App {
  ...
  */
  ...
  println(listOfIntegers.map((element: Int) => element * 2).toString) // alternative way =   _ * 2

  println(listOfIntegers.filter((element: Int) => element % 2 == 0).toString) // alternative way = _ % 2 == 0
  ...
  val forCombinations: MyList[String] = for {
    n <- listOfIntegers
